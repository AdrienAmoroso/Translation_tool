...
class ExcelService:
    ...
    def load_segments_from_sheet(self, sheet_name: str) -> List[Segment]:
        ...
        if not has_target:
            logger.warning(f"[{sheet_name}] Target column '{self.config.translation.target_col}' not found")
        if not has_comment:
        ...
        for idx, row in df.iterrows():
            ...
            source = str(row[self.config.translation.source_lang]) if pd.notna(row[self.config.translation.source_lang]) else ""
            target = str(row[self.config.translation.target_col]) if (has_target and pd.notna(row[self.config.translation.target_col])) else ""
            comment = str(row[self.config.excel.comment_column]) if (has_comment and pd.notna(row[self.config.excel.comment_column])) else ""
